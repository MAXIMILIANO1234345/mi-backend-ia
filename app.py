...
print("Iniciando API (Versión Grafo 7.0 - Anti-Caos)... Cargando variables.")
load_dotenv()
...
generative_model_name = "models/gemini-2.5-flash" # ¡Rápido y con límite alto!
...
def get_plan_de_busqueda(pregunta_usuario, driver, modelo_gemini):
    ...
    Si la pregunta es muy amplia (como 'cómo modelar'), devuelve TODOS los capítulos que contengan la palabra "modelado" o "modelar".
    ...
    try:
        ...
        # Asegurarnos de que devuelve una lista
        if 'capitulos_objetivo' not in plan or not isinstance(plan['capitulos_objetivo'], list):
             return {"capitulos_objetivo": []}
        return plan
    except Exception as e:
        print(f"Error en el Planificador (Paso 1): {e}.")
        return {"capitulos_objetivo": []} # Fallar a una lista vacía

# PASO 2: EL INVESTIGADOR (Busca CON FILTRO o SIN FILTRO)
def buscar_en_grafo(db_driver, vector_pregunta, capitulos_objetivo):
    """
    Busca en Neo4j. Si hay capítulos, filtra. Si no, hace una búsqueda general.
    """
    ...
    with db_driver.session(database=NEO4J_DATABASE) as session:
        ...
        # Esta es la consulta base de búsqueda vectorial
        cypher_base = "CALL db.index.vector.queryNodes('chunk_vector_index', $limit, $vector) YIELD node AS item, score"
        
        # Esta es la parte que une y devuelve los datos
        cypher_return = """
        ...
        if not capitulos_objetivo:
            # Plan B: El Planificador falló. Hacemos una búsqueda simple PERO LIMITADA (3 chunks).
            # Esto evita que el Redactor se contamine con demasiada basura.
            print("Paso 2: Investigador - Planificador no dio capítulos. Haciendo búsqueda simple (Top 3).")
            cypher_query = f"{cypher_base} {cypher_return}"
            result = session.run(cypher_query, vector=vector_pregunta, limit=3)
        
        else:
            # Plan A: El Planificador funcionó. Hacemos una búsqueda amplia (Top 10)
            # y LUEGO la filtramos por los capítulos correctos.
            print("Paso 2: Investigador - Aplicando filtro de capítulos.")
            cypher_filtro = "MATCH (item)-[:PERTENECE_A]->(c:Capitulo) WHERE c.titulo IN $capitulos_objetivo"
            cypher_query = f"{cypher_base} {cypher_filtro} {cypher_return}"
            result = session.run(cypher_query, vector=vector_pregunta, limit=10, capitulos_objetivo=capitulos_objetivo)
...
# PASO 3: EL REDACTOR (PROMPT ESTRICTO, sin cambios)
def generar_respuesta_final(pregunta_usuario, contexto_items, modelo_gemini):
    ...
    5.  Al final de tu respuesta (si la encontraste en el contexto), cita tu fuente:
        "{fuente_para_prompt}"
...
# --- 4. El "Endpoint" de Flask (¡Lógica de respaldo mejorada!) ---
...
def manejar_pregunta():
    ...
    try:
        ...
        # PASO 2: Investigar
        # (La función buscar_en_grafo ahora maneja la lógica de "lista vacía" por sí misma)
        contexto_items = buscar_en_grafo(driver, vector_pregunta, capitulos)
