...
def buscar_nodos_conceptuales(pregunta_usuario):
    ...
    Encuentra los conceptos clave (ej: 'Cycles', 'F12') relacionados con la pregunta.
    """
    vector = get_text_embedding(pregunta_usuario)
    if not vector: return []

    try:
        # Usamos la función RPC 'buscar_nodos' que creamos en el SQL del grafo
        response = supabase.rpc('buscar_nodos', {
            'query_embedding': vector,
            'match_threshold': 0.55,  # Umbral más alto para encontrar conceptos exactos
            'match_count': 5
        }).execute()
...
def expandir_relaciones(nodos_encontrados):
    ...
    Dado los conceptos encontrados, busca con qué están conectados.
    Ej: Si encontró 'Cycles', busca qué 'REQUIERE' o qué 'ES'.
    """
    ...
    try:
        # Consultamos la tabla 'relaciones'. 
        # Queremos saber: Nodo Origen -> Relación -> Nodo Destino
        # Nota: Hacemos un JOIN manual simple para obtener los nombres
        ...
        # Mapeo rápido de ID a Nombre para el origen
        ...
        if response.data:
            for rel in response.data:
                ...
                # Accedemos a los datos del nodo destino gracias a la relación en Supabase
                ...
                # Formato legible para la IA: "Cycles --REQUIERE--> GPU"
...
def generar_respuesta_grafo(pregunta, nodos, relaciones, modelo):
    ...
    La IA recibe hechos estructurados y conceptos definidos.
    """
    
    # 1. Preparamos definiciones de conceptos encontrados
    definiciones = "\n".join([f"* {n['nombre']}: {n['descripcion']}" for n in nodos])
    
    # 2. Preparamos la red de conexiones
    conexiones = "\n".join(relaciones) if relaciones else "Sin conexiones directas encontradas."

    if not nodos:
        return {
            "respuesta_principal": "No encontré conceptos relacionados en mi base de conocimientos de grafos.",
            "puntos_clave": [],
            "fuente": "Grafo vacío"
        }

    prompt = f"""
    Eres un experto en Blender 3D que razona usando una Red de Conocimiento (Grafo).
    ...
    Instrucciones:
    1. Usa las 'Conexiones' para explicar CAUSA y EFECTO (ej: Si A requiere B, explícalo).
    2. Usa las 'Definiciones' para explicar qué son las cosas.
    3. Si el grafo muestra pasos (ej: F12 --ACCIONA--> Render), úsalos como instrucciones.
    
    Salida JSON:
    {{
        "respuesta_principal": "Explicación basada en la lógica del grafo...",
        "puntos_clave": ["Dato 1", "Conexión importante"],
    ...
    try:
        ...
        return {"respuesta_principal": "Error de razonamiento.", "puntos_clave": [], "fuente": "Sistema"}

# ==============================================================================
# === FLUJO 2: GENERADOR DE SCRIPTS (Igual que antes) ===
# ==============================================================================
...
def health_check():
    return jsonify({"status": "online", "mode": "GraphRAG (Nodos + Relaciones)"}), 200
